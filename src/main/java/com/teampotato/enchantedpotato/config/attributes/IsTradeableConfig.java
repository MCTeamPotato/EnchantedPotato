...
public class IsTradeableConfig {
    public IsTradeableConfig() {
        File isTradeableConfig = new File(EarlySetupInitializer.attributesDir, "isTradeable.json");
        if (!isTradeableConfig.exists()) {
            try {
                FileWriter writer = writeFile(isTradeableConfig);
                writer.close();
            } catch (Exception e) {
